...
where `msg` is a value with of type
 ...
 interface InternetIdentityAuthRequest {
   ...
   sessionPublicKey: Uint8Array; // TODO: what encoding is this?
   maxTimeToLive?: bigint;
 }

where

* the `sessionPublicKey` contains the public key of the session key pair. NOTE: can be anything supported by IC
...
--
6. Now the client application window expects a message back, with data `event`. TODO: WRONG! `.origin` is on the `message`, not `message.data`.
